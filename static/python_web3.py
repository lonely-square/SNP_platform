...
# 打开文件
workbook = xlrd.open_workbook(r'/usr/local/nginx/html/report/static/Score_SC_valid_1.xlsx')
# 获取sheet2                      
...
def createReport():
    ...
    sickerReport(document, paragraph, sheet2)

    # 时间路径
    # t = datetime.datetime.now().strftime('%Y%m%d%H%M')
    doc_base_path = "/usr/local/nginx/html/report/static"
    dname = "demo.docx"  # word文档文件名
    doc_path = os.path.join(doc_base_path, dname)  # word生成路径
    # 生成word文档
    document.save(doc_path)

    pdf_base_path = "/usr/local/nginx/html/report/static"
    pname = "demo.pdf"  # pdf名
    ...
    if os.path.exists(doc_base_path) and os.path.exists(pdf_base_path):
        if os.path.isfile(doc_path) and os.path.isfile(pdf_path):  # 判断是否存在该word和pdf文件
            ...
            os.system("libreoffice7.2 --headless --convert-to pdf %s --outdir %s" % (doc_path,pdf_base_path))
...
def createSickTable(document, paragraph, sheet):
    nlen = sheet.nrows
    clen = sheet.ncols
    ...
    # 设置表格第一行内容和格式
    hdr_cells[0].text = '疾病'
    hdr_cells[1].text = '相对风险度'
    hdr_cells[0].font = Pt(24)
    hdr_cells[0].color = RGBColor(0, 0, 0)
    hdr_cells[1].font = Pt(24)
    hdr_cells[1].color = RGBColor(0, 0, 0)
    # 循环遍历表格每一行，输入数据
    for i in range(nlen - 1):
        row_cells = table.add_row().cells
        row_cells[0].text = str(sheet2.cell(i + 1, 0).value)
        row_cells[1].text = str(sheet2.cell(i + 1, 2).value)


def sickerReport(document, paragraph, sheet2):
    paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.RIGHT  # 靠右
    document.add_heading('基因检测报告', 0).bold = True  # 文档标题
    ...
    createSickTable(document, paragraph, sheet2)
